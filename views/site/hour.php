    ...
    <div class="content warningHourMain">
        ...
        <div class="search">
            <? $form = ActiveForm::begin([
                'action' => Url::to(['/site/hour-search'])
            ]); ?>

            <?= $form->field($model, 'search')
                ->textInput(['placeholder' => "Поиск в разделе..."])
                ->label(false) ?>
        ...
        if ($str) {
            ...
            Показаны результаты поиска по запросу: <span>\"{$str}\"</span>
        </div>";
        }
        ?>
    </div>
...
$i = 0;
if (empty($hours)) {
    echo "<div class='greyBg clear'><div class='content'>
        <div class='noResultSearch'>
            Поиск не дал результатов. Попробуйте что-то изменить
        </div>
    </div></div>";
} else {
